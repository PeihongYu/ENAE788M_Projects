...
def main():
    ...
    for i in range(3):
        bag = rosbag.Bag(bag_files[i], "r")
        topics = bag.get_type_and_topic_info()[1].keys()
        print(topics)
        dir = bag_files[i].split('.')[0] + '/'
        os.mkdir(dir)

        # raw_data_topics = ['/imu0', '/imu1', '/mavros/imu/data_raw', '/mavros/imu/mag']
        ...
        for _, msg, t in bag.read_messages(topics=topic):
            ...
            data = tuple([t.to_time()]) + magnetic_field + magnetic_field_covariance
            f.write(str(data).replace('(', '').replace(')', '') + '\n')
        f.close()
